...
class AsyncOAuthFlow:
    ...
    async def build_authorize_url_redirection(self, request: BoltRequest, state: str) -> BoltResponse:
        return BoltResponse(
            status=302,
            headers={
    ...
    async def handle_callback(self, request: BoltRequest) -> BoltResponse:
        ...
        if installation is None:
            ...
            return await self.build_callback_failure_response(request, reason="invalid_code", status=401)

        # persist the installation
        try:
            await self.store_installation(request, installation)
        except BoltError as e:
            return await self.build_callback_failure_response(request, reason="storage_error", error=e)
    ...
    async def store_installation(self, request: BoltRequest, installation: Installation):
        # may raise BoltError
